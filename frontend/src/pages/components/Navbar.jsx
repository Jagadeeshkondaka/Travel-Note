...
const Navbar = ({ userinfo, searchTerm, onSearch }) => {
  ...
  return (
    <div className='bg-white flex flex-col sm:flex-row items-center justify-between px-4 sm:px-6 lg:px-10 py-3 gap-4 sm:gap-2'>
      ...
      {isToken && (
        <div className='flex items-center gap-2 w-full sm:flex-1 sm:w-auto'>
          <input
            type="text"
            placeholder="Search stories..."
            value={searchTerm}
            onChange={(e) => onSearch(e.target.value)}
            className="border rounded px-3 py-2 w-full sm:w-48 lg:w-64 text-sm outline-none focus:border-cyan-400"
          />
